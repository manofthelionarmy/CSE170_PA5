...
void gl_print_info ( GsOutput* out )
{
	...
	if ( !int(glGetString) ) { o << "OpenGL not initialized.\n"; return; } // protection
...
{
	static int v=-1;
	if ( v<0 && int(glGetString) ) v = extractversion ( glGetString(GL_VERSION) );
	return v;
}
...
{ 
	static int v=-1;
	if ( v<0 && int(glGetString) ) v = extractversion ( glGetString(GL_SHADING_LANGUAGE_VERSION) );
	return v;
}
