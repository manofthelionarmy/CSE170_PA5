...
{
	_accel = -1;
	if ( !l || !l[0] ) { _accel=-1; _text.set(0); set_size(" "); return; }
	if ( !chkaccel ) { _accel=-1; _text.set(l); set_size(_text); return; }

	// UiDev: accelerators in labels are being detected and correctly underlined,
	// but still missing to implement in UiManager the keyboard events

	int i, len=strlen(l);
	if ( len>12 ) len=12; // do not search long labels fully
	for ( i=0; i<len; i++ ) if ( l[i]=='&' ) { _accel=i; break; }
	if ( _accel<0 )
	{	_text.set ( l ); }
	else
	{	GsString s(l);
