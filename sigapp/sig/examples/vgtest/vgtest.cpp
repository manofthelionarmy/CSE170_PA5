...
{
	float r=0.2f; float dang=GS_TORAD(45);
...
void VgViewer::build_scene ()
{
	...
	// path has to be computed first because it alters the graph when endpoints change:
	if ( _vpbut->value() )
	{	GsPolygon path;
		float cost = _vg.search_shortest_path ( _pi, _pg, path );
		_path->line_width ( 2.0f );
		_path->push ( GsColor(10,190,10) );
		_path->push_polygon ( path );
		message().setf ( "Nodes:%d  Cost:%f", path.size(), cost );
	}
...
int VgViewer::handle ( const GsEvent& e )
{
	if ( e.alt && e.button1 && (e.type==GsEvent::Push||e.type==GsEvent::Drag) )
	{	if ( dist(_pi,e.mousep)<dist(_pg,e.mousep) )
		{	_pi=e.mousep; } else { _pg=e.mousep; }
		build_scene();
		redraw();
ws_check();
		return 1;
