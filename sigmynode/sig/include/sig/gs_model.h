...
{ public :
	...
	GsArray<GsPnt2>		T;  //!< List of texture coordinates (indexed by Ft)
	GsArray<Face>		Ft; //!< Indices to the texture coords in T
	GsArray<GsMaterial> M;  //!< List of materials
	GsArray<Group>		G;  //!< Groups of faces with same material and texture (size is 0 or M.size())
	...
	enum GeoMode { Empty,  //!< No F or V arrays defined, the model is empty
				   ...
				   Hybrid  //!< F, V, N, and Fn defined, normals indexed by indices in Fn: Fn.size()==F.size()
				 };
	...
	/*! Will be set to true if a texture map has been defined in the per-group materials */
	gscbool textured;
